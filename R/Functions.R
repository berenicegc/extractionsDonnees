...
import <- function(path = ".", file = "donnees", TaxRef = "taxref"){
  directory <- setwd(dir = path)
  donnees <<- read_delim(list.files(directory, pattern = file, full.names = TRUE)
                         [which.max(file.mtime(list.files(directory, pattern = file, full.names = TRUE)))], delim = ";")
  taxref <<- read_delim(list.files(directory, pattern = TaxRef), delim = ",")

  #Contrôle import données
  if(length(donnees)==0){
    cli_alert_danger(col_red("Erreur : données non importées. Veuillez relancer la fonction import().
                                    Si le problème persiste, vérifiez le nom du fichier (argument file) ainsi que le chemin (argument path)."), wrap = TRUE)}
...
extract <- function(col = c("plante", "caste", "station", "annee_determination", "methode"), precision_taxo = NULL){
  ...
  if(length(which(str_detect("caste", col) == TRUE))==1){
    ...
    export_final_donnees <<- export_final_donnees %>%
      cbind(caste)
  ...
  if(length(which(str_detect("plante", col) == TRUE))==1){
    ...
    donnees %>%
      ...
      select(-lb_nom_plante) -> donnees_plantes
    ...
    suppressMessages(full_join(donnees_plantes, temp_file_extraction_sp)) %>%
  ...
  if(!is.null(precision_taxo)){
    if(length(which(str_detect("plante", col) == TRUE))==1 & str_detect("famille|genre|sp", precision_taxo)){
      if(precision_taxo == "famille"){
        export_final_donnees %>%
          filter_at(vars(plante_cd_ref,plante_genre),any_vars(!is.na(.))) ->> export_final_donnees
      }
      ...
      if(precision_taxo == "genre"){
        export_final_donnees %>%
          filter(plante_cd_ref %in% taxref$cd_ref[which(taxref$id_rang %in% c("GN", "SSGN", "SC", "SBSC", "SER", "SSER", "AGES", "ES",
                                                                              ...
                                                                              "FOES", "LIN","CLO", "RACE", "CAR", "MO"))]) ->> export_final_donnees
      }
      ...
      if(precision_taxo == "sp"){
        export_final_donnees %>%
          filter(plante_sp %in% taxref$lb_nom[which(taxref$id_rang %in% c("ES", "SMES", "MES", "SSES","NAT", "VAR",
                                                                          "SVAR", "FO", "SSFO", "FOES", "LIN",
                                                                          "CLO", "RACE", "CAR", "MO"))]) ->> export_final_donnees
      }
    }
...
#'
#' @details Par défaut, le fichier est appelé "export_final_donnees.csv" et est stocké au même endroit que le script R.
...
export <- function(path = ".", fichier = "export_final_donnees.csv"){
  write.csv(x = export_final_donnees, file = paste0(path, "/", fichier))
}
